...
def num_img_fams_list():
  inp_dir = r"/share/home/104188/handsome_le/Malware/Win/Datasets/malimg_paper_dataset_imgs"
...
def train():
    ...
    for i in range(kfold):
        ...
        y_train_onehot = keras.utils.np_utils.to_categorical(y_train, num_classes=len(no_imgs))
        #print(y_train[100:200])
        #print("y_train.shape", y_train.shape)
        X_test = X[test_indices].reshape((-1, 256, 256, 1)).astype(float)
        y_test = y[test_indices]
        #print(y_test[100:200])
        y_test_onehot = keras.utils.np_utils.to_categorical(y_test, num_classes=len(no_imgs))

        model = model_cnn(no_imgs)
        # Compile model
        model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy'], name="markov_cnn")
        ...
        # Fit the model
        model.fit(X_train, y_train_onehot, epochs=1, batch_size=1, verbose=1, callbacks=[tensorboard_callback])
    ...
    markov_log_dir = "model/markov"  + datetime.datetime.now().strftime("%Y%m%d-%H%M")
    model.save(markov_log_dir + '.h5')
